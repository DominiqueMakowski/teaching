...
model <- brms::brm(qsec ~ mpg, data = mtcars) 
parameters::parameters(model)
```
```{r}
#| echo: true
...
- ESS: Effective Sample Size: how many "good iterations" (truly independent draws) we have. Should be > 400 (the maximum being the number of iterations that we darw).  
## Voilà!
...
- We can reshape the dataframe (pivot longer) to have one column for all iterations and another to identify the iteration number
...
get_predicted(model_priors2, data=newdata, iterations=250) |> 
  as.data.frame() |> 
  reshape_iterations() |> 
  cbind(newdata) |> 
  ggplot(aes(x=mpg)) +
...
- In general, don't panic and follow the rule of "good enough"
  ...
  - If not, go with the default / flat / very weakly informative priors. 
