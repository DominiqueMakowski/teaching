...
- Frequentist "inferential" statistics are used to make inferences about the **parent population** from which the data are **sampled**
...
- Pros: 
  ...
  - It is important mathematical properties
- Cons:
...
that typically exclude 2.5% from each tail of the distribution and always include
...
- It works well for symmetric distributions, but can be problematic for skewed distributions
  - It can exclude "high probability" regions of the distribution, which is counter-intuitive
...
- The *pd* tells us if the probability of a distribution (of an effect) is positive (> 0) or negative (< 0)
