...
## Bayesian Models: Quick n' Dirty {.center background-color="#003B49"}
...
## Bayesian Models: Priors {.center background-color="#003B49"}
...
p0b <- ggplot(prior2, aes(x = x, y = y)) + 
  geom_area(orientation = "y", alpha=0.8, fill="orange") +
  scale_x_reverse() +
  labs(y="qsec", x="Density", title  = "Normal(M, 10*SD)") + 
...
## Chill out, it's just a prior {background-color="#1A237E"}
...
- Predictions^[More accurately, "expected" values] can be obtained using `estimate_relation()`
...
```{r}
#| echo: false
modelbased::estimate_relation(model_priors2, keep_iterations=250) |>
  reshape_iterations() |> 
  ggplot(aes(x=mpg)) +
...
![](img/priors_table.png)

[Daniel Lüdecke, Anna Makowski, Jens Klein, and Dominique Makowski (2026). *Choosing informative priors in Bayesian regression models. A simulation study and tutorial using Stan and R*. Preprint](https://doi.org/10.21203/rs.3.rs-7599542)
