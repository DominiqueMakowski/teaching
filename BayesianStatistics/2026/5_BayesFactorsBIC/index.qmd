...
In R, functions can have aliases (i.e., two names for the same function):
...
- Note that because BFs can be very big or very small, they are sometimes presented on the log scale ($log~BF_{10}$)
- Importantly, $BF = 1$ (no evidence either side) corresponds to $log~BF = 0$
- Easy interpretation: negative $log~BF$ numbers mean $BF$ < 1 mean evidence **against**
