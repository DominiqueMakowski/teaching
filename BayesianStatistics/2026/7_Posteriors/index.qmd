...
- Make sure you have successfuly made `brms` work (see last week's instructions). If not, get in touch!
...
- When re-used (in other analyses, meta-analyses, ...), it inevitably carries a loss of information (e.g., uncertainty).
  - E.g., computing the Bayesian correlation within each participant between RT and number of errors, and then using the "point-estimate" to further relate it to personality traits^[Unfortunately, *propagating uncertainty*, i.e., re-using estimations of uncertainty in follow-up analyses, is not common practice (and is technically challenging)]
