...
- "What is the probability of you learning something new during this course?"
- Typical conceptualization of probabilities applies to *series*, not to single or non-repeatable events
...
- The **Product Rule of probability**: $P(A, B) = P(B)~P(A|B) = P(A)~P(B|A)$
  - <sub><sup>*The probability that it rains AND I am late equals the probability that it rains today, multiplied by how often I am late when it rains in general.*</sup></sub>
...
posterior <- posterior / sum(posterior)  # Normalize so that total == 100%
posterior
```
...
- What's the likelihood of the "tails" outcome? 
...
likelihood_tails <- 1 - likelihood_heads  # The "opposite", since there are only 2 possible outcomes
likelihood_tails
```
...
data.frame(cointypes=cointypes, y=likelihood_heads) |> 
  ggplot(aes(x=cointypes, y=y)) +
  geom_line(color="red") + 
  geom_point(size=3, color="red") +
  geom_line(data = data.frame(cointypes=cointypes, y=likelihood_tails), color="darkorchid") + 
  geom_point(data = data.frame(cointypes=cointypes, y=likelihood_tails), size=3, color="darkorchid") +
  scale_x_continuous(breaks=cointypes, 
                     labels = paste0(cointypes, "\n", c("Biased", "Imperfect", "Fair", "Imperfect", "Biased"))) +
  labs(x="Coin type", y="Likelihood of heads (red) or tails (purple)") +
  theme_bw()
```
