...
- More generally, a linear model finds the **parameters** of the best fitting normal distribution 
  - The location $\mu$ = which technically corresponds to the mean of the data
  - The spread $\sigma$ = the standard deviation of the "residuals" (i.e., the distance between the observed data and the predicted mean)
...
- The line that goes through the *Gaussian* distribution's locations $\mu$ - i.e., the result of a MLE model - is also the line that minimizes the sum of squared residuals (OLS)
...
- The line that minimizes the sum of squared residuals (OLS) = the solution to the Gaussian model MLE algorithm
- **USEFUL!**
