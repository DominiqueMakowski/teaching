...
- The likelihood term $P(data|H)$ is often hard to compute for Bayesian models (in which the parameters are themselves probabilistic), but not impossible
  - Efficient approaches exist for simple models
  - Computationally expensive for complex models
- However, defining "the null hypothesis" $H_0$ (and thus $P(data|H_0)$) in probabilistic terms is not trivial at all
- As a result, getting the "Bayes Factor" of whether my result is significant ($H_1$ vs. $H_0$) is typically difficult, but it is possible to compute Bayes factors efficiently and adequately for "simple" tests
...
- True Bayes factor (i.e., not approximations using Frequentist models) can be easily computed for "simple" tests, such as *t*-tests
...
- What about priors?
  - In this particular case, let's just say that they are good enough "default" priors (there is established literature justifying it). Can be used "as-is" without too much worry\*
  - \*Some people will be mad lose it if they see this
...
- The stronger the BF, the stronger the "difference" between the **prior** and the **posterior**
  - *"Before seeing the data, we think H0 is 3:1 more likely (prior), but after seeing the data, H1 is 9:1 more likely."* The updating factor is the BF 
...
## Recap of Differences {background-color="#1A237E"}
...
<!-- TODO: retrieve quote that illustrates this -->
::::
...
- **Bayesian Statistics** 🥰️
  ...
  - Compute likelihood of the data for each hypothesis $p(data|H)$
    - Can be used to compare different hypotheses (Bayes Factors)
