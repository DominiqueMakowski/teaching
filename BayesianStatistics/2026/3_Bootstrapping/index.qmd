...
- We try to infer what would happen if we repeated the experiment (i.e., repeatedly sampled from the parent distribution) an infinite number of times
- We quantify the assumed variability of the estimated parameter: the **standard error** <sup><sub>(standard error = variability of sample means; standard deviation = variability of a sample).</sub></sup> 
- Under the CLT, we often assume that this distribution of the estimate is Normal. Not to be confused with the distribution of the "standardized parameter" (e.g., *t*-value) under the null-hypothesis - used to compute *p*-values.
- This standardized index of precision (e.g., *t*-value) is used to find answers interpreted as "significance" (e.g., probability of being superior to a threshold) using **analytical distributions** (e.g., a *t*-distribution with certain parameters).
...
- However, I still personally recommends 95% 
  - See [**https://github.com/easystats/bayestestR/discussions/250**](https://github.com/easystats/bayestestR/discussions/250)
